...
type FormAddBlogPropsType = {
  actionAddBlog: (formData: FormData) => Promise<{
    message: string
    error: boolean
  }>
}
...
const ThumpNail = ({
  ...
  return (
    <div className='w-fit flex relative' onMouseOver={onShowDeleteIcon} onMouseOut={onHideDeleteIcon}>
      {/** TODO change img tag to Next built-in Image component */}
...
const FormAddBlog: FC<FormAddBlogPropsType> = ({ actionAddBlog }) => {
  ...
  const handleAddBlog = async (formData: FormData) => {
    const result = await actionAddBlog(formData)
    if (result?.error) {
