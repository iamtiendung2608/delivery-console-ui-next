...
type FormAddBlogPropsType = {
  actionAddPaymentMethod: (formData: FormData) => Promise<{
    message: string
    error: boolean
  }>
}

const FormAddPaymentMethod: FC<FormAddBlogPropsType> = ({ actionAddPaymentMethod }) => {
  const router = useRouter()

  const handleAddPaymentMethod = async (formData: FormData) => {
    const result = await actionAddPaymentMethod(formData)
    if (result?.error) {
  ...
  return (
    <form action={handleAddPaymentMethod}>
      ...
      <div className='mb-5.5 flex gap-x-3'>
