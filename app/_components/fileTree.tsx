...
export function FileTree({ exampleName, filePaths, code, isIframe }: Props) {
  ...
  const paths = filePaths
    ...
    .sort((a, b) => {
      if (b.includes('_components') || b.includes('_utils') || b.includes('_hooks')) {
        return -1
      }
  ...
  const initialPath = filePathFromParams ?? (paths.includes(initialCandidate) ? initialCandidate : paths[0] ?? '')
    ...
    return Object.keys(node).map(key => {
        ...
        return (
          <li
            ...
            className={[
              ...
              newPath.replace(basePathWithName, '') === selectedPath ? 'text-blue-300' : ''
            ].join(' ')}
          >
            <button
              type='button'
      ...
      return (
        ...
        >
          ...
          <ul className='ml-2 t-1 border-l border-l-gray-600'>{renderTree(node[key], i + 1, newPath)}</ul>
        </details>
      )
    })
  }
  ...
  return (
    ...
    >
      <div className='min-w-max overflow-y-auto pr-6 max-h-48 lg:max-h-none mb-2 lg:mb-0'>{renderTree(tree, 0)}</div>
